...
function resultHamali(v, i) {
    ...
    var totalCount = parseFloat($("#totalCounttxt").val());


    (v.value === "0" || v.value === "") ? emptyValue() : fillValue();

    function emptyValue() {
        var addRound = totalCount;
        var totalCount = Math.round(totalCount);
        addRound = Math.abs(addRound - totalCount);
        $('#addRoundTxt').val(addRound.toFixed(2));
        $("#netAmountTxt").val(totalCount);
        $("#hamaliTxt").val(00);
    ...
    function fillValue() {
        ...
        var hamaliTotal = (totalCount + Hamali);
        var addRound = hamaliTotal;
        rhamaliTotal = Math.round(hamaliTotal);
        addRound = Math.abs(addRound - rhamaliTotal);
        // $("#totalCounttxt").val(hamaliTotal);
        $("#addRoundTxt").val(addRound.toFixed(2))
        $("#netAmountTxt").val(rhamaliTotal);
...
function resultCashDisc(v, i) {
    ...
    function emptyValue() {
        ...
        var addRound = ans;
        rans = Math.round(ans);
        addRound = Math.abs(addRound - rans);
        $("#addRoundTxt").val(addRound.toFixed(2));    
        $("#totalCounttxt").val(ans);
        $("#netAmountTxt").val(rans);
    ...
    function fillValue() {
        ...
        var a = (totalCount * Discount / 100);
        var cashDiscTotal = totalCount - a;
        var addRound = cashDiscTotal;
        rcashDiscTotal = Math.round(cashDiscTotal);
        addRound = Math.abs(addRound - rcashDiscTotal);
        // $("#totalCounttxt").val(cashDiscTotal);
        $("#addRoundTxt").val(addRound.toFixed(2));
        $("#netAmountTxt").val(rcashDiscTotal);
