...
function resultDiscount(v, i) {
    ...
    function emptyValue() {
        ...
        // $("#totalDisc").html(num2);
        $("#netAmt").html("Net = " + num2.toFixed(2));
        $("#netAmountTxt").val(Math.round(num2));
    ...
    function fillValue() {
        ...
        $("#netAmt").html("Net = " + total.toFixed(2));
...
function resultAddless(v, i) {
    ...
    if (sym == "-") {
        ...
        $("#netAmt").html("Net = " + taxableCount.toFixed(2));
        ...
        if (SGST > 0) {
            ...
            $("#demonetAMT").val(totalGSTAmt + tot);
            $("#GSTAA" + num).val(taxableCount + (answithGST * 2));
        // Calculate Round Of  ------------------------------------------- @tilesh
            var net = totalGSTAmt + tot;
            var round = net;
            
            $("#netAmt").html("Net = " + (net.toFixed(2)));
            net = Math.round(net);
            round = Math.abs(round - net);
            
            $("#addRound").html("Add Round = " + (round.toFixed(2)));
            $("#addRoundTxt").val(round.toFixed(2));
            $("#netAmountTxt").val(Math.round(net));
            ...
            var ansfinalGSTAmt = (totalIGSTAmount + totalTaxableAmount);
            // alert(ansfinalGSTAmt)
            // Calculate Round Off value --------------------------- @tilesh
            var round = ansfinalGSTAmt;
            $("#netAmt").html("Net = " + ansfinalGSTAmt.toFixed(2));
          
            ansfinalGSTAmt = Math.round(ansfinalGSTAmt);
            
            round = Math.abs(round - ansfinalGSTAmt);
...
function clearText(i) {
    ...
    afterClearTotal = Math.round(afterClearTotal);
    $("#netAmountTxt").val(afterClearTotal.toFixed(2));
    $("#demonetAMT").val(afterClearTotal.toFixed(2));
    $("#netAmt").html("Net = " + afterClearTotal.toFixed(2));
...
function itemName(i) {
    ...
    var incl = $('#item option').filter(function () {
        return this.value == val;
    }).data('incl');

    var incl_rate = parseFloat(incl);
    var rate = parseFloat(xyz);
    ...
    // alert(totalTax);     
    // $("#MRP" + nums).val(xyz);
    $("#incl_rate_price" + nums).val(rate);
    $("#incl_rate" + nums).val(incl);
    $("#GST" + nums).val(tax);
    ...
    // Calculate Rate ,Incl Rate through
    // console.log("Rate = ".rate);
    // console.log("INCL Rate = ".incl_rate);

    var ans = (rate / incl_rate);
    // var finalans = ans.toFixed(2);
    $("#MRP" + nums).val(ans.toFixed(2));

    // console.log("Total = ".ans);

};

function calculateInclRate(i) {

    var incl_rate_price = parseFloat($("#incl_rate_price" + i).val());
    var incl_rate = parseFloat($("#incl_rate" + i).val());

    var total = (incl_rate_price / incl_rate)

    $("#MRP" + i).val(total.toFixed(2));
