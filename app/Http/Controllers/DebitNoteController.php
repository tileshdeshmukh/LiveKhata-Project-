...
use App\Models\Debit_Note_data;
...
class DebitNoteController extends Controller
{
    ...
    public function search_dn(Request $request){
      $input = trim($request->stext);
      $search['search_data'] = DebitNote::where('customer_name', 'like', "%{$input}%")->orWhere('mobile', 'like', "%{$input}%")->orderBy('created_at', 'desc')->get();
      $search['url'] = 'debitNote';
  ...
  {   
    ...
    $data['taxInvoiceService'] = DebitNote::orderBy('id', 'ASC')
  ...
  $data['taxInvoiceServiceFetch1'] = Debit_Note_data::where("debit_note_id", $i)
                                                ->where('cmp_id', $compay_id)
                                                ->get();
  
  $data['taxInvoiceServiceFetch'] = DebitNote::where("Voucher_no",$i)
                                                ->where("cmp_id", $compay_id)
                                                ->first();
  ...
  //   return $saveServiceBill;
    ...
    if($saveServiceBill=="saveTaxInvoice"){
              ...
              //   $Description=$request->Description;
                ...
                {
                    $data=new Debit_Note_data();
                    $data->debit_note_id=$service_bill_id;
                    $data->cmp_id=$cmp_id;
    ...
    // -------------------------------------------------
        ...
        $res=DebitNote::where('Voucher_no',$id)
                        ->where('cmp_id', $compay_id)
                        ->delete();

        $res1=Debit_Note_data::where('debit_note_id',$id)
                        ->where('cmp_id', $compay_id)
                        ->delete();
        ...
        $taxInvoiceService = new DebitNote();
    ...
    $data['taxInvoiceServiceFetch'] = DebitNote::where("cmp_id", $compay_id)
   ...
   $data['taxInvoiceServiceFetch1'] = DB::table('debit_note_data')
                                      ->rightjoin('product_trees','product_trees.id','debit_note_data.itemId')
                                      ->where("cmp_id", $compay_id)
