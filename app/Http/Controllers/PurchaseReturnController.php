...
use App\Models\Purchase_Return_data;
...
class PurchaseReturnController extends Controller
{
      ...
      public function search_pr(Request $request){
        ...
        $search['search_data'] = PurchaseReturn::where('customer_name', 'like', "%{$input}%")->orWhere('mobile', 'like', "%{$input}%")->orderBy('created_at', 'desc')->get();
        $search['url'] = 'purchaseReturn';
  ...
  {   
    ...
    $data['taxInvoiceService'] = PurchaseReturn::orderBy('id', 'ASC')
  ...
  $data['taxInvoiceServiceFetch1'] = Purchase_Return_data::where("purchase_return_id", $i)
                                                ->where('cmp_id', $compay_id)
                                                ->get();
  
  $data['taxInvoiceServiceFetch'] = PurchaseReturn::where("Voucher_no",$i)
                                                ->where("cmp_id", $compay_id)
                                                ->first();
  ...
  //   return $saveServiceBill;
    ...
    if($saveServiceBill=="saveTaxInvoice"){
                ...
                $serviceBill = new PurchaseReturn();
              ...
              //   $Description=$request->Description;
                ...
                // $hamali=array_filter($request->hamali);
                ...
                {
                    $data=new Purchase_Return_data();
                    $data->purchase_return_id=$service_bill_id;
                    $data->cmp_id=$cmp_id;
    ...
    // -------------------------------------------------
        ...
        $res=PurchaseReturn::where('Voucher_no',$id)
                        ->where('cmp_id', $compay_id)
                        ->delete();

        $res1=Purchase_Return_data::where('purchase_return_id',$id)
                        ->where('cmp_id', $compay_id)
                        ->delete();
        ...
        $taxInvoiceService = new PurchaseReturn();
    ...
    return redirect("/purchaseReturn/taxServiceBillInvoice/$service_bill_id");
  }
    ...
    {                      
      ...
      $data['billName']="Purchase Return";

      $voucherId= $id;
    
      $compay_id= Auth::user()->id;
      
    $data['taxInvoiceServiceFetch'] = PurchaseReturn::where("cmp_id", $compay_id)
   ...
   $data['taxInvoiceServiceFetch1'] = DB::table('purchase_return_data')
                                      ->rightjoin('product_trees','product_trees.id','purchase_return_data.itemId')
                                      ->where("cmp_id", $compay_id)
