...
class ReceiptsController extends Controller
{
    ...
    public function showServiceBill($i=0)
    {
        ...
        $data['serviceBill'] = Receipts::orderBy('id', 'ASC')
      ...
      $data['serviceBillFetch1'] = Receipts_data::where("receipts_id", $i)
                                                    ->where('cmp_id', $compay_id)
                                                    ->get();
      
      $data['serviceBillFetch'] = Receipts::where("Voucher_no",$i)       
                                                    ->where("cmp_id", $compay_id)
                                                    ->first();
    ...
    public function addServiceBill(Request $request){
        ...
        if($saveServiceBill=="saveServiceBill"){
                    ...
                    {
                        $data=new Receipts_data();
                        $data->receipts_id=$receipts_id;
                        $data->cmp_id=$cmp_id;
        ...
        // Update Service Bill
            ...
            $res=Receipts::where('Voucher_no',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();

            $res1=Receipts_data::where('receipts_id',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();
    ...
    $data['serviceBillFetch'] = Receipts::where("cmp_id", $compay_id)
...
//                                                     ->get();
  ...
  $data['serviceBillFetch1'] = DB::table('receipts_data')
                                ->rightjoin('product_trees','product_trees.id','receipts_data.itemId')
                                ->where("cmp_id", $compay_id)
