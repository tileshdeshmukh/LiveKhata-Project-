...
use App\Models\CompositionBill;
use App\Models\Composition_bill_data;
...
class compositionBillController extends Controller
{
  ...
  public function search_com(Request $request){
    ...
    public function showServiceBill($i=0)
    {
        ...
        $billCount = count($ids);
        if($i){
           return $data['last_id']=(int)$i;
            $data['bill_last_id']=$billCount+1;
        
      $data['serviceBillFetch1'] = Composition_bill_data::where("composition_bill_id", $i)
                                                    ->where('cmp_id', $compay_id)
                                                    ->get();
      
      $data['serviceBillFetch'] = CompositionBill::where("Voucher_no",$i)       
                                                    ->where("cmp_id", $compay_id)
                                                    ->first();
    ...
    public function addServiceBill(Request $request){
        ...
        if($saveServiceBill=="saveServiceBill"){
                    $cmp_id= Auth::user()->id;
                    $composition_bill_id=$request->Voucher_no;
                    ...
                    {
                        $data=new Composition_bill_data();
                        $data->composition_bill_id=$composition_bill_id;
                        $data->cmp_id=$cmp_id;
                    ...
                    $serviceBill = new CompositionBill();
        ...
        // Update Service Bill
            ...
            $res=CompositionBill::where('Voucher_no',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();

            $res=Composition_bill_data::where('composition_bill_id',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();
    ...
    {         
        ...
        $data['billName']="Composition Bill";
        
      $voucherId= $id;
      $compay_id= Auth::user()->id;

    $data['serviceBillFetch'] = CompositionBill::where("cmp_id", $compay_id)
    ...
    // $data['serviceBillFetch1'] = Composition_bill_data::where("cmp_id", $compay_id)
    //                                                 ->where('composition_bill_id',  $voucherId)
    //                                                 ->get();    


    $data['serviceBillFetch1'] = DB::table('composition_bill_data')
                            ->rightjoin('product_trees','product_trees.id','composition_bill_data.itemId')
                            ->where("cmp_id", $compay_id)
                            ->where('composition_bill_id',  $voucherId)    
