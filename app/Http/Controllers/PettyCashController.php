...
class PettyCashController extends Controller
{
    ...
    public function showServiceBill($i=0)
    {
        ...
        $data['serviceBill'] = PettyCash::orderBy('id', 'ASC')
      ...
      $data['serviceBillFetch1'] = PettyCash_data::where("pettyCash_id", $i)
                                                    ->where('cmp_id', $compay_id)
                                                    ->get();
      
      $data['serviceBillFetch'] = PettyCash::where("Voucher_no",$i)       
                                                    ->where("cmp_id", $compay_id)
                                                    ->first();
    ...
    public function addServiceBill(Request $request){
        ...
        if($saveServiceBill=="saveServiceBill"){
                    ...
                    {
                        $data=new PettyCash_data();
                        $data->pettyCash_id=$receipts_id;
                        $data->cmp_id=$cmp_id;
        ...
        // Update Service Bill
            ...
            $res=PettyCash::where('Voucher_no',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();

            $res1=PettyCash_data::where('pettyCash_id',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();
    ...
    $data['serviceBillFetch'] = PettyCash::where("cmp_id", $compay_id)
...
//                                                     ->get();
  ...
  $data['serviceBillFetch1'] = DB::table('pettyCash_data')
                                ->rightjoin('product_trees','product_trees.id','pettyCash_data.itemId')
                                ->where("cmp_id", $compay_id)
