...
use App\Models\Tax_Invoice_Service_data;
use App\Models\TaxInvoiceService;
...
class TaxInvoiceServiceController extends Controller
{
    ...
    {   
      ...
      $data['taxInvoiceService'] = TaxInvoiceService::orderBy('id', 'ASC')
    ...
    $data['taxInvoiceServiceFetch1'] = Tax_Invoice_Service_data::where("tax_invoice_service_id", $i)
                                                  ->where('cmp_id', $compay_id)
                                                  ->get();
    
    $data['taxInvoiceServiceFetch'] = TaxInvoiceService::where("Voucher_no",$i)       
                                                  ->where("cmp_id", $compay_id)
                                                  ->first();
    ...
    //   return $saveServiceBill;
      ...
      {                      
        ...
        $data['billName']="Tax Invoice Service";
  
        $voucherId= $id;
        $compay_id= Auth::user()->id;
  
      $data['taxInvoiceServiceFetch'] = TaxInvoiceService::where("cmp_id", $compay_id)
      ...
      $data['taxInvoiceServiceFetch1'] = DB::table('tax_invoice_service_data')
                                        ->rightjoin('product_trees','product_trees.id','tax_invoice_service_data.itemId')
                                        ->where("cmp_id", $compay_id)
                                        ->where('tax_invoice_service_id',  $voucherId)    
