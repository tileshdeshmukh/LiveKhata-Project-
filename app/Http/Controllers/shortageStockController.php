...
use App\Models\ShortageStock;
use App\Models\Shortage_Stock_data;
...
class shortageStockController extends Controller
{
  ...
  public function search_ss(Request $request){
    ...
    public function showServiceBill($i=0)
    {
        ...
        $data['serviceBill'] = ShortageStock::orderBy('id', 'ASC')
      ...
      $data['serviceBillFetch1'] = shortage_stock_data::where("shortage_stock_id", $i)
                                                    ->where('cmp_id', $compay_id)
                                                    ->get();
      
      $data['serviceBillFetch'] = ShortageStock::where("Voucher_no",$i)       
                                                    ->where("cmp_id", $compay_id)
                                                    ->first();
    ...
    public function addServiceBill(Request $request){
        ...
        if($saveServiceBill=="saveServiceBill"){
                    $cmp_id= Auth::user()->id;
                    $shortage_stock_id=$request->Voucher_no;
                    ...
                    {
                        $data=new shortage_stock_data();
                        $data->shortage_stock_id=$shortage_stock_id;
                        $data->cmp_id=$cmp_id;
        ...
        // Update Service Bill
            ...
            $res=ShortageStock::where('Voucher_no',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();

            $res=shortage_stock_data::where('shortage_stock_id',$id)
                            ->where('cmp_id', $compay_id)
                            ->delete();
    ...
    {         
        ...
        $data['billName']="Shortage Stock";
        
      $voucherId= $id;
      $compay_id= Auth::user()->id;

    $data['serviceBillFetch'] = ShortageStock::where("cmp_id", $compay_id)
    ...
    // $data['serviceBillFetch1'] = shortage_stock_data::where("cmp_id", $compay_id)
    //                                                 ->where('shortage_stock_id',  $voucherId)
    //                                                 ->get();    

    $data['serviceBillFetch1'] = DB::table('shortage_stock_data')
                                ->rightjoin('product_trees','product_trees.id','shortage_stock_data.itemId')
                                ->where("cmp_id", $compay_id)
                                ->where('shortage_stock_id',  $voucherId)    
